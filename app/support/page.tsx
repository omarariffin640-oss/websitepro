...
export default function SupportPage() {
    ...
    const getStatusBadge = (status: Ticket["status"]) => {
        if (status === "open") {
            return <Badge className="border-green-500/30 bg-green-500/20 text-green-400">Open</Badge>;
        }

        if (status === "in_progress") {
            return <Badge className="border-yellow-500/30 bg-yellow-500/20 text-yellow-400">In Progress</Badge>;
        }

        return <Badge className="border-red-500/30 bg-red-500/20 text-red-400">Closed</Badge>;
    };
    ...
    return (
        <DashboardShell>
            ...
            <div className="grid grid-cols-1 gap-6 xl:grid-cols-3">
                <Card className="border-purple-500/20 bg-gradient-to-b from-purple-500/10 to-gray-950 xl:col-span-1">
                    <CardHeader>
                    ...
                    <CardContent className="space-y-4">
                        ...
                        <Button onClick={submitTicket} className="w-full rounded-xl bg-purple-500 py-6 text-white hover:bg-purple-600">
                            Submit Ticket
                        </Button>
                    </CardContent>
                </Card>
                ...
                <Card className="border-white/10 bg-white/5 xl:col-span-2">
                    ...
                    <CardContent>
                        <div className="space-y-3">
                            {tickets.map((ticket) => (
                                <div key={ticket.id} className="rounded-xl border border-white/10 bg-black/40 p-4 transition hover:border-purple-500/40 hover:bg-purple-500/10">
                                    ...
                                    <p className="text-sm leading-relaxed text-gray-400">{ticket.message}</p>
                                </div>
                            ))}
                        </div>
                    </CardContent>
...
function HelpCard({ icon: Icon, title, text }: any) {
    return (
        <Card className="border-white/10 bg-white/5 transition hover:border-purple-500/40 hover:bg-purple-500/10">
            <CardContent className="p-5">
