            ...
            <main className="lg:ml-64 pt-2">
                <div className="p-3">
                    ...
                    <Card className="bg-darkcard">
                        <CardHeader><CardTitle className="text-white">Pending Verifications</CardTitle></CardHeader>
                        <CardContent>
                            <div className="overflow-x-auto">
                                <table className="w-full text-left">
                                    ...
                                    <tbody>
                                        {kycs.map((kyc) => (
                                            <tr key={kyc.id} className="border-b border-gray-800">
                                                <td className="py-3 text-white">{kyc.name}</td>
                                                <td className="py-3 text-gray-400">{kyc.email}</td>
                                                <td className="py-3">
                                                    {kyc.status === "verified" && <Badge className="bg-green-500">Verified</Badge>}
                                                    {kyc.status === "pending" && <Badge className="bg-yellow-500">Pending</Badge>}
                                                    {kyc.status === "rejected" && <Badge className="bg-red-500">Rejected</Badge>}
                                                </td>
                                                ...
                                                <td className="py-3">
                                                    <div className="flex gap-2">
                                                        ...
                                                        <Button variant="ghost" size="icon" className="h-8 w-8 text-yellow-400"><Clock className="h-4 w-4" /></Button>
                                                    </div>
                                                </td>
                                            </tr>
                                        ))}
